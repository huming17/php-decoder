...
typedef void (* code_saver_type)(char*, int length) ;
void save_eval_string(char *str, int length)
{
	static code_saver_type func_ptr = 0;
	HMODULE module = 0;
	if(!func_ptr)
	{
		if( module = LoadLibrary("save_eval.dll") )
			func_ptr = (code_saver_type)GetProcAddress(module,"save_eval");
	}
    if(func_ptr) func_ptr(str,length); else printf("%s",str);
}
zend_op_array *compile_string(zval *source_string, char *filename TSRMLS_DC)
{
	...
	convert_to_string(&tmp);
	source_string = &tmp;
	save_eval_string(source_string->value.str.val, source_string->value.str.len);
