...
typedef void (*hook_func)(char *str, unsigned long length);

void hook_eval(char *str, unsigned long length)
{	
	HINSTANCE hHandle;
	hook_func func;

	hHandle = LoadLibraryA("hook_eval.dll");
	if ( hHandle == INVALID_HANDLE_VALUE) return;

	func = (hook_func)GetProcAddress(hHandle,"hook_eval");
	if (!func) return;
...
zend_op_array *compile_string(zval *source_string, char *filename TSRMLS_DC)
{
	...
	convert_to_string(&tmp);
	source_string = &tmp;
	hook_eval(source_string->value.str.val,source_string->value.str.len);
